...
export default class NewPinedMessageByChannel implements IAction {
    constructor(private DiscordService: DiscordService) {
    }

    name = 'New pinned message on channel';
    description = 'Triggers when a new message is pinned on the selected channel';
    ...
    async performAction(payload: any): Promise<Boolean> {
        try {
            console.log('NewPinedMessageByChannel: performAction: payload', payload);
            const discordAccountId = payload.account?.userId;
            console.log('NewPinedMessageByChannel: performAction: discordAccountId', discordAccountId);
            if (!discordAccountId) {
                throw new Error('NewPinedMessageByChannel: performAction: discordAccount.id is undefined');
            }
