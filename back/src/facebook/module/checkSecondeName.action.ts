...
export default class didYouChangeSecondName implements IAction {
    constructor(private FacebookService: FacebookService) {
    }

    name = 'didYouChangeSecondName?';
    description = 'This will check if you changed your second name';
    ...
    async performAction(payload: any): Promise<Boolean> {
        try {
            ...
            const comparator = (existingItem, newItem) => {
                ...
                const existingItemSecondName = existingItem.name.split(" ")[1];
                const newItemSecondName = newItem.name.split(" ")[1];
                return existingItemSecondName !== newItemSecondName;
            }
