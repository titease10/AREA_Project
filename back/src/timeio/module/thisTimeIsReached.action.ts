...
export default class thisTimeIsReached implements IAction {
    constructor(private TimeioService: TimeioService) {
    }

    name = 'does we have reached the time';
    ...
    async performAction(payload: any): Promise<Boolean> {
        ...
        try {
            console.log('thisTimeIsReached: performAction: payload', payload);
            //check if the minut and hour are correctly defined
            if (!payload.city) {
                throw new Error('thisTimeIsReached: performAction: city is not defined');
            }
            if (!payload.zone) {
                throw new Error('thisTimeIsReached: performAction: zone is not defined');
            }
            //check if the minut and hour are correctly defined
            if (!payload.hours || payload.hours > 24 || payload.hours < 0) {
                throw new Error('thisTimeIsReached: performAction: hours is not defined');
            }
            if (!payload.minutes || payload.minutes > 60 || payload.minutes < 0) {
                throw new Error('thisTimeIsReached: performAction: minutes is not defined');
            }
            ...
            console.log('thisTimeIsReached: performAction: result', result);
            const hours = payload.hours;
            const minutes = payload.minutes;
            const date = new Date();
            const currentHours = date.getHours();
            const currentMinutes = date.getMinutes();
            //we want to check if we are between an hours that is higher than the one we want to check
